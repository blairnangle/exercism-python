...
def approx_average_is_average(hand):
    ...
    first_and_last_avg = (sorted_hand[0] + sorted_hand[len(sorted_hand) - 1]) / 2

    return actual_avg == first_and_last_avg or actual_avg == median(hand)
...
def average_even_is_average_odd(hand):
    ...
    evens = [card for card in hand if hand.index(card) % 2 == 0]
    odds = [card for card in hand if hand.index(card) % 2 != 0]
